...
export function remainingMinutesInOven(actualMinutesInOven) {
  const resRemain = EXPECTED_MINUTES_IN_OVEN - actualMinutesInOven;
  return resRemain;
}
...
export function preparationTimeInMinutes(numberOfLayers) {
  const resPrep = numberOfLayers * PREPARATION_MINUTES_PER_LAYER;
  return resPrep;
}
...
export function totalTimeInMinutes(numberOfLayers, actualMinutesInOven) {
  const resTot = preparationTimeInMinutes(numberOfLayers) + actualMinutesInOven;
  return resTot;
}
